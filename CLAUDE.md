# u1-hw-camera: instructions for AI assistants
...
This repo publishes one plugin: **Camera HW Accel** for the Snapmaker U1, hardware-accelerated MJPEG +
WebRTC streaming for the MIPI and USB cameras, tapping the Rockchip SoC video pipeline instead of
CPU-encoding every frame.

It carries Python (the MPP stream server) alongside hand-written C sources and an arm64 build
toolchain that compiles the binaries the plugin ships.
...
4. **Run the gate and make it green:** `bash scripts/check.sh`. The gate needs the `lib_bespok3d`
   ...
   workflow-pinning, the em-dash guard, and shellcheck. It also runs ruff and pytest over the plugin's
   Python (see `scripts/check.sh` for the exact targets).
