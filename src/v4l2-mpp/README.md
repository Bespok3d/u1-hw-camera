...
| stream-http | HTTP server for camera streaming (snapshots, MJPEG, H264, browser player); entry point `camera-stream.py` |
...
Python apps (stream-http's `camera-stream.py`, detect-http) require no compilation.
